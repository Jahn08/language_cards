	...
	Widget build(BuildContext context) {
		...
		return new Column(children: [
			...
			new ValueListenableBuilder(
				...
				builder: (_, AppTheme theme, __) => new _AppearanceSettingsSection(theme, (newTheme) {
			...
			new ValueListenableBuilder(
				...
				builder: (_, bool isDirty, label) => new ElevatedButton(
					child: label,
					onPressed: isDirty
						? () async {
...
class _StudySettingsSectionRow extends StatelessWidget {
	...
	const _StudySettingsSectionRow(
		{this.options, this.inintialValue, this.onValueChanged, this.label});
			...
			new Expanded(
				child: new Container(
					child: new StyledDropdown(options,
						label: label, initialValue: inintialValue, onChanged: onValueChanged),
	...
	Widget build(BuildContext context) => new Row(
		children: UserParams.interfaceLanguages
			.map((ln) => new _LanguageOption(
				lang: ln, isSelected: ln == interfaceLang, onTap: () => onLangChanged(ln)))
			.toList());
}
...
class _LanguageOption extends StatelessWidget {
	...
	@override
	Widget build(BuildContext context) => new GestureDetector(
		child: new IconOption(icon: AssetIcon.getByLanguage(lang), isSelected: isSelected),
		onTap: () {
			if (isSelected) return;
...
class _HelpSectionBody extends StatelessWidget {
	...
	Widget build(BuildContext context) {
		...
		new _TextButtonedRow(
			locale.mainScreenStudyModeMenuItemLabel, () => Router.goToStudyHelp(context)),
		new _TextButtonedRow(
			locale.mainScreenWordPacksMenuItemLabel, () => Router.goToPackHelp(context)),
		new _TextButtonedRow(
			locale.mainScreenWordCardsMenuItemLabel, () => Router.goToCardHelp(context))
		]);
	}
}
	...
	new FutureLoader(Configuration.getParams(context, reload: false), (AppParams params) {
		...
		new _TextButtonedRow(
			...
			() async => (await EmailLink.build(
					...
					subject:
						locale.barScaffoldSettingsPanelContactsSectionSuggestionEmailSubject))
				.activate()),
		new _TextButtonedRow(
...
Widget build(BuildContext context) => new _SubSectionHeader(
	AppLocalizations.of(context).barScaffoldSettingsPanelLanguageSectionLabel);
}
	...
	Widget build(BuildContext context) => new Row(children: [
			new Padding(
				...
				child: new Text(title,
					textAlign: TextAlign.left, style: const TextStyle(fontSize: Consts.biggerFontSize)))
		]);
}
...
class _TextButtonedRow extends StatelessWidget {
	...
	Widget build(BuildContext context) => new Row(children: <Widget>[
			new Flexible(
				child: new SizedBox(
					child: new TextButton(child: new Text(label), onPressed: onPressed), height: 40.0))
		]);
}
	...
	Widget build(BuildContext context) =>
		new Row(mainAxisAlignment: MainAxisAlignment.center, children: [
			new Padding(
				...
				child: new Text(title, style: const TextStyle(fontSize: Consts.largeFontSize)))
		]);
}
...
class _SettingsOpenerButton extends StatelessWidget {
	...
	Widget build(BuildContext context) {
		return new IconButton(
			icon: const Icon(Icons.settings), onPressed: () => Scaffold.of(context).openDrawer());
	}
}
		...
		child: new Scaffold(
			...
			appBar: onNavGoingBack == null
				...
				: new NavigationBar(_title,
					actions: barActions, onGoingBack: onNavGoingBack, leading: _settingsOpener),
			body: body,
			bottomNavigationBar: bottomBar,
			floatingActionButton: floatingActionButton
		),
		onWillPop: onNavGoingBack == null ? null: 
			() {
				onNavGoingBack();
