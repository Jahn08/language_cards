...
class _BottomBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => new BottomAppBar(
      child: new Row(
          children: optionsBuilder(context),
...
class _MapNotifier<TKey, TValue> extends ValueNotifier<Map<TKey, TValue>> {
  ...
  void remove(TKey key) {
    if (value.remove(key) == null) return;
...
abstract class ListScreenState<TItem extends StoredEntity,
          ...
          new IconButton(
              ...
              icon: new ValueListenableBuilder(
                  ...
                  builder: (_, bool isEditorMode, __) =>
                      new Icon(isEditorMode ? Icons.edit_off : Icons.edit))),
              ...
              if (isEditorMode)
                return new ValueListenableBuilder(
                    ...
                    builder: (_, Map<int, _CachedItem<TItem>> markedItems, __) {
                      final items =
                          markedItems.values.map((e) => e.item).toList();
                      return new _BottomBar((scaffoldContext) =>
  ...
  bool _getIsSearchModeAvailable(int itemsLength) =>
      curFilterIndex != null ||
      itemsLength >= ListScreen.searcherModeItemsThreshold;
  ...
  @protected
  List<Widget> getBottomBarOptions(List<TItem> markedItems,
      ...
      [
        new IconedButton(
            ...
            onPressed: () async {
              ...
              final continueRemoval = await shouldContinueRemoval(
                  markedItems.values.map((v) => v.item).toList());
              if (!continueRemoval) return;
        ...
        new IconedButton.labelWidget(
            ...
            labelWidget: new ValueListenableBuilder(
                ...
                builder: (_, __, ___) => new Text(getSelectorBtnLabel(
                    allSelected: removableItemsLength ==
                        _markedItemsNotifier.value.length,
                    locale: locale))),
            onPressed: () {
              final allSelected =
                  removableItemsLength == _markedItemsNotifier.value.length;
              if (allSelected) {
                ...
                builder: (_, List<TItem> items, __) {
                    ...
                    return new ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          return new ValueListenableBuilder(
                              valueListenable: _markedItemsNotifier,
                              builder: (_,
                                      Map<int, _CachedItem<TItem>> markedItems,
                                      __) =>
                                  _buildCheckListItem(context, index,
                                      items[index], markedItems.keys.toSet()));
                        },
                  ...
                  return new ListView.builder(
                      ...
                      itemBuilder: (context, index) =>
                          _buildDismissibleListItem(
                              context, index, locale, items[index]),
                      controller: _scrollController);
                });
          }));

  Widget _buildCheckListItem(BuildContext buildContext, int itemIndex,
      TItem item, Set<int> markedIds) {
  ...
  Widget _buildListTile(BuildContext buildContext, TItem item) => new ListTile(
  ...
  Widget _buildDismissibleListItem(BuildContext buildContext, int itemIndex,
      AppLocalizations locale, TItem item) {
  ...
  Future<void> _deleteMarkedForRemoval(List<int> ids) async {
    if (_itemsToRemove.isEmpty) return;

    final itemsToDelete = _getCachedItemsMarkedForRemoval(ids);
    final removedIndexes =
        itemsToDelete.map((i) => i.item.textData[0]).toList();
    _deleteFilterIndexes(removedIndexes);

    _deleteFromMarkedForRemoval(ids);
    deleteItems(_extractItemsForDeletion(itemsToDelete));

    final isSearchOff = _isSearchModeNotifier.value &&
        !_getIsSearchModeAvailable(_itemsLengthFromIndexes);
    if (isSearchOff) _isSearchModeNotifier.value = false;

    if (curFilterIndex != null &&
        !_filterIndexesNotifier.value.containsKey(curFilterIndex))
      await refetchItems();
    else
  ...
  void _deleteFilterIndexes(List<String> removedIndexes) {
    final grouppedIndexes =
        removedIndexes.fold<Map<String, int>>({}, (res, val) {
