...
class _CardEditorDialog
    extends OutcomeDialog<MapEntry<StoredWord, StoredPack>> {
...
class _StudyScreenState extends State<StudyScreen> {
  ...
  void _setNextCard() => _controller.nextPage(
  ...
  void _setIndexCard(int newIndex) {
    ...
    if (_curCardIndexNotifier.value == indexCard) return;
  ...
  Widget build(BuildContext context) {
    ...
    return new Card(
        ...
        margin: const EdgeInsets.only(
            left: spaceSize, right: spaceSize, bottom: spaceSize),
        child: new InkWell(
          ...
          new Row(children: [
            new Container(
                margin: const EdgeInsets.all(5),
                child: new Text(
                    WordStudyStage.stringify(card.studyProgress, locale)))
          ]),
          TightFlexible(
              child: new Center(
                  child: new ListTile(
...
class _ButtonPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final buttons = [
      new ElevatedButton(
          child:
              new _CenteredBigText(locale.studyScreenLearningCardButtonLabel),
