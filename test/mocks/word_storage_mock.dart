...
class _WordDataProvider extends DataProviderMock<StoredWord> {
  ...
  HashSet<String> get intFieldNames => _intFieldNames ??= super.intFieldNames
    ..addAll([StoredWord.packIdFieldName, StoredWord.studyProgressFieldName]);
}
...
class WordStorageMock extends WordStorage {
  ...
  static List<StoredWord> _generateWords(int length, int? parentsOverall,
      {String Function(String, int?)? textGetter}) {
    const cardsWithoutPackNumber = 5;
    final cardsWithPackNumber = length - cardsWithoutPackNumber;
    final words = new List<StoredWord>.generate(
        cardsWithPackNumber,
        (index) => generateWord(
            id: index + 1,
            packId: parentsOverall == null
                ? null
                : (index < parentsOverall ? index : index % parentsOverall) + 1,
            textGetter: textGetter));
    words.addAll(new List<StoredWord>.generate(
        cardsWithoutPackNumber,
        (index) => generateWord(
            id: cardsWithPackNumber + index + 1,
            hasNoPack: true,
            textGetter: textGetter)));
  ...
  static StoredWord generateWord(
    ...
    return new StoredWord(textGetter?.call(text, id) ?? text,
        ...
        translation: new List<String>.generate(
                Randomiser.nextInt(5) + 1, (index) => Randomiser.nextString())
            .join('; '),
        ...
        packId: hasNoPack
            ...
            : packId ??
                Randomiser.nextInt(
                        parentsOverall ?? PackStorageMock.namedPacksNumber) +
                    1,
  ...
  Future<StoredWord?> updateWordProgress(int id, int studyProgress) async {
    ...
    final cards = await upsert([
      new StoredWord(wordToUpdate.text,
          id: wordToUpdate.id,
