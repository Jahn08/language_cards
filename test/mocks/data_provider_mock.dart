...
abstract class DataProviderMock<T extends StoredEntity> extends DataProvider {
  ...
  Future<List<Map<String, dynamic>>> fetch(String tableName,
    ...
    if (take != null) propItems = propItems.take(take);
  ...
  Future<List<DataGroup>> _groupBySeveral(String tableName,
    ...
    const valSeparator = '|';
    return Future.value(_getFilteredItemProps(filters)
        .fold<Map<String, int>>({}, (map, i) {
          ...
          if (vals.length != groupFieldsOverall) return map;
  ...
  HashSet<String> get intFieldNames =>
      _intFieldNames ??= new HashSet<String>.from([StoredEntity.idFieldName]);
