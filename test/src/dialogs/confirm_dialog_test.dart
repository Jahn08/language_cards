...
void main() {
  testWidgets(
      'Renders a dialog with confirmation/cancellation buttons returning expected results',
      (tester) async {
    final expectedTitle = Randomiser.nextString();
    await _testDialogRendering(
        tester,
        (title, content) => new ConfirmDialog(
            title: title, content: content, confirmationLabel: expectedTitle),
        {true: expectedTitle, false: _cancellationLabel});
  });

  testWidgets(
      'Renders a dialog with the only an OK button returning the true value',
      (tester) async {
    await _testDialogRendering(
        tester,
        (title, content) =>
...
Future<void> _testDialogRendering<T>(
    WidgetTester tester,
  ...
  actions.values
      .forEach((label) => _assertTextVisibility(label, isVisible: true));
