...
void main() {
  ...
  testWidgets('Shows the word dialog according to items passed as an argument',
      (tester) async {
    final availableWords = new List<Word>.generate(
        Randomiser.nextInt(5) + 1, (_) => _buildRandomWord());
    ...
    await dialogTester.testRenderingOptions(availableWords, (finder, word) {
      ...
      expect(
          find.descendant(
              of: finder, matching: find.text(word.translations.join('; '))),
          findsOneWidget);
    }, ListTile);
  });
...
WordSelectorDialog _buildDialog(BuildContext context) =>
    new WordSelectorDialog(context);
