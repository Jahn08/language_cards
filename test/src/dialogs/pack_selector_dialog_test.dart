...
void main() {
  testWidgets('Shows all named packs to choose from in the dialog',
      (tester) async {
    final storage = new PackStorageMock(packsNumber: 20);
    final chosenPack = storage.getRandom();

    final dialogTester = new SelectorDialogTester(
        tester, (context) => _buildDialog(context, chosenPack.id));
    await dialogTester.testRenderingOptions(
