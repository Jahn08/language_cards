...
void main() {
  ...
  testWidgets(
      'Shows the translation dialog according to items passed as an argument',
      (tester) async {
  ...
  testWidgets('Returns chosen translations and hides the dialog',
      (tester) async {
    final availableItems =
        Randomiser.nextStringList(minLength: 5, maxLength: 15);
    ...
    await assistant.tapWidget(optionFinders.at(chosenOptionIndex + 1),
        atCenter: true);

    const int anotherChosenOptionIndex = 3;
    await assistant.tapWidget(optionFinders.at(anotherChosenOptionIndex + 1),
        atCenter: true);
  ...
  testWidgets('Returns all translations chosen by ticking the title checkbox',
      (tester) async {
    final availableItems =
  ...
  testWidgets(
      'Returns no translation after choosing nothing by clicking the title checkbox twice',
      (tester) async {
    final dialogTester = new SelectorDialogTester(tester, _buildDialog);
  ...
  testWidgets(
      'Returns null after tapping the cancel button of the translation dialog',
      (tester) async {
    final dialogTester = new SelectorDialogTester(tester, _buildDialog);
