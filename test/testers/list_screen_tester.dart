...
class ListScreenTester<TEntity extends StoredEntity> {
  ...
  const ListScreenTester(
      String screenName, ListScreen<TEntity> Function([int?]) screenBuilder)
      : screenName = screenName + ' List Screen',
        _screenBuilder = screenBuilder;

  void testEditorMode() {
    testWidgets(_buildDescription('switches to the editor mode and back'),
        (tester) async {
    ...
    testWidgets(
        _buildDescription('selects and unselects all items in the editor mode'),
        (tester) async {
      ...
      expect(
          getSelectorBtnLabel(tester),
          Localizator.defaultLocalization
              .constsSelectAll(itemsOverall.toString()));
    });

    testWidgets(
        _buildDescription(
            'removes nothing when no items have been selected in the editor mode'),
        (tester) async {
      await pumpScreen(tester);
      ...
      int index = 0;
      final items = new Map.fromEntries(tester
          .widgetList<CheckboxListTile>(tilesFinder)
      ...
      final listTiles =
          tester.widgetList<CheckboxListTile>(tilesFinder).toList();
    ...
    testWidgets(_buildDescription('removes selected items in the editor mode'),
        (tester) async {
      final itemsToRemove =
    ...
    testWidgets(_buildDescription('keeps selection made right after removal'),
        (tester) async {
      await _testRemovingItemsInEditor(tester, selectSomeItemsInEditor);
      ...
      assureSelectionForAllTilesInEditor(tester,
          selected: true, onlyForSomeItems: true);
    });

    testWidgets(
        _buildDescription('recovers removed items in their previous order'),
        (tester) async {
      final itemsToRemove =
          await _testRemovingItemsInEditor(tester, selectSomeItemsInEditor);
    ...
    testWidgets(
        _buildDescription('recovers all removed items in their previous order'),
        (tester) async {
      final itemsToRemove =
          await _testRemovingItemsInEditor(tester, (assistant) async {
        await deactivateEditorMode(assistant);

        int index = 0;
        final itemsToCheck = new Map.fromEntries(tester
            .widgetList<ListTile>(
                AssuredFinder.findSeveral(type: ListTile, shouldFind: true))
            .map(
                (t) => new MapEntry(index++, _extractTitle(tester, t.title!))));
    ...
    testWidgets(
        _buildDescription(
            'resets selected items after quitting the editor mode'),
        (tester) async {
      await pumpScreen(tester);
  ...
  Future<BaseStorage<TEntity>> pumpScreen(WidgetTester tester,
      [Future<void> Function(BaseStorage<TEntity>)? beforePumping,
      int? itemsNumber]) async {
  ...
  Finder assureSelectionForAllTilesInEditor(WidgetTester tester,
    ...
    expect(
        onlyForSomeItems
            ? enabledItems.any((w) => w.value == selected)
            : enabledItems.every((w) => w.value == selected),
        true);
  ...
  String _extractTitle(WidgetTester tester, Widget title) {
    final textWidgetFinder =
        find.descendant(of: find.byWidget(title), matching: find.byType(Text));
  ...
  Future<void> _confirmRemovalIfNecessary(WidgetAssistant assistant,
      ...
      bool shouldNotWarn = false}) async {
  ...
  Future<void> _assureRecoveredItems(
    ...
    expect(checkTiles.every((t) => t.value != true), true);
  ...
  Future<void> _undoRemoval(WidgetAssistant assistnat) async {
    final undoBtnFinder =
        AssuredFinder.findOne(label: 'Undo', shouldFind: true);
    await assistnat.tapWidget(undoBtnFinder);
  }

  Future<Map<int, String>> selectSomeItemsInEditor(WidgetAssistant assistant,
      [int? chosenIndex]) async {
    final tilesFinder = find
        .byWidgetPredicate((w) => w is CheckboxListTile && w.onChanged != null);
    ...
    final middleTileIndex =
        chosenIndex == null || chosenIndex == 0 || chosenIndex == lastTileIndex
            ? (lastTileIndex / 2).round()
            : chosenIndex;
    ...
    for (final indexToSelect in tileIndexesToSelect) {
      ...
      if (resultItems.containsKey(index)) continue;

      final finder = tilesFinder.at(indexToSelect);
      resultItems[index] =
          _extractTitle(tester, tester.widget<CheckboxListTile>(finder).title!);
  ...
  void testDismissingItems() {
    Future<void> forEachDismissible(
        int itemsNumber, Future<void> Function(Finder) processor) async {
      final dismissibleFinders = tryFindingListItems(shouldFind: true);
      for (int i = 0; i < itemsNumber; ++i)
        await processor(find
            .ancestor(of: find.byType(ListTile), matching: dismissibleFinders)
            .last);
    }

    Future<void> assertItemsRemoval(WidgetAssistant assistant,
        List<String> removedTitles, BaseStorage<TEntity> storage) async {
      await assistant.pumpAndAnimate(500);

      for (final title in removedTitles) {
        AssuredFinder.findOne(type: ListTile, label: title);

        final storedItems = await assistant.tester
            .runAsync(() => storage.fetch(textFilter: title));
        expect(storedItems!.isEmpty, true);
      }
    }

    testWidgets(_buildDescription('removes items by dismissing'),
        (tester) async {
      final storage = await pumpScreen(tester);
      final itemsOverall = await tester.runAsync(() => storage.count());
      ...
      expect(
          getSelectorBtnLabel(tester).contains(expectedCardsNumber.toString()),
          true);
    });

    testWidgets(
        _buildDescription('recovers an item from some of dismissed ones'),
        (tester) async {
      final storage = await pumpScreen(tester);
      ...
      await forEachDismissible(3, (dismissible) async {
        ...
        if (removedTitles.isEmpty || recoveredTitle != null) {
          ...
          AssuredFinder.findOne(
              type: ListTile, label: recoveredTitle, shouldFind: true);
        }
      });
  ...
  Future<String> _removeByDismissing(
      WidgetAssistant assistant, Finder dismissible) async {
    final tester = assistant.tester;
  ...
  void testSearchMode(TEntity Function(int) newEntityGetter,
      ...
      final storage = await pumpScreen(tester, (st) async {
        ...
        if (items.length > _searcherModeThreshold) {
          final idsToDelete = items
              .take(items.length - _searcherModeThreshold + 1)
              .map((i) => i.id)
              .toList();
    ...
    testWidgets(
        _buildDescription(
            'filters items by an index in the search mode and resets filtering after closing the mode'),
        (tester) async {
      ...
      Finder filteredTilesAssurer() {
        ...
        expect(
            filteredTiles.every(
                (t) => _extractTitle(tester, t.title!).startsWith(filterIndex)),
            true);
    ...
    testWidgets(
        _buildDescription(
            'deactivates index filter and deletes it when its items are deleted with the active search mode'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold);
      ...
      indexGroups.map((g) => g.key).forEach((i) {
        assureFilterIndexActiveness(tester, i);
      });
    });

    testWidgets(
        _buildDescription(
            'deletes an index filter when its items are deleted and the search mode is active'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold);
      ...
      await activateEditorMode(assistant);
      final titlesToDelete = (await tester
              .runAsync(() => storage.fetch(textFilter: filterIndexToDelete)))!
          .map((i) => i.textData)
          .toList();
      ...
      _findFilterIndex(filterIndexToDelete);
      indexGroups.remove(filterGroupToDelete);
      assureFilterIndexes(indexGroups.map((g) => g.key).toList(),
          shouldFind: true);
    });

    testWidgets(
        _buildDescription(
            'deletes an index filter when its items are deleted with the search mode turned off'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold,
    ...
    testWidgets(
        _buildDescription(
            'keeps an index filter active when it still has items left after deletion in the active search mode'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold);
      ...
      await activateEditorMode(assistant);
      final titlesToDelete =
          (await tester.runAsync(() => storage.fetch(textFilter: filterIndex)))!
              .take(1)
              .map((i) => i.textData)
              .toList();
      await selectItemsInEditor(assistant, titlesToDelete);
      ...
      expect(
          getSelectorBtnLabel(tester),
          Localizator.defaultLocalization
              .constsSelectAll(leftItemsNumber.toString()));
    });

    testWidgets(
        _buildDescription(
            'hides the search mode button when the mode is inactive, and the list has got too short'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold);
    ...
    testWidgets(
        _buildDescription(
            'quits the search mode and hides its button when the list has got too short, and there is no active filter'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold);
    ...
    testWidgets(
        _buildDescription(
            'hides the search mode button after deactivating the mode when the list has got too short, and there is an active filter index'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold);
      ...
      for (final gr in indexGroups) {
        ...
        if (overallLength < _searcherModeThreshold) break;
      }
    ...
    testWidgets(
        _buildDescription('deactivates an index filter when clicking it twice'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold);
      ...
      await chooseFilterIndex(assistant, filterIndex);
      assureFilterIndexActiveness(tester, filterIndex);
    ...
    testWidgets(
        _buildDescription('resets scrolling when changing an index filter'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold);
      ...
      ScrollController scroller =
          assistant.retrieveListViewScroller(tilesFinder);
    ...
    Future<void> selectAndAssureSelection(
        WidgetAssistant assistant, int expectedItemsOverall) async {
      await selectAll(assistant);
      ...
      expect(
          getSelectorBtnLabel(tester),
          Localizator.defaultLocalization
              .constsUnselectAll(expectedItemsOverall.toString()));
      assureSelectionForAllTilesInEditor(tester, selected: true);
    }

    testWidgets(
        _buildDescription(
            'resets selection and a number of items available for selection when changing an index filter'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold);
      ...
      expect(
          getSelectorBtnLabel(tester),
          Localizator.defaultLocalization
              .constsSelectAll(firstIndexGroup.value.toString()));
    ...
    testWidgets(
        _buildDescription(
            'does not reset selection and a number of items available for selection when exiting the search mode without choosing any indexes'),
        (tester) async {
      final storage = await _pumpScreenWithEnoughItems(tester,
          newEntityGetter: newEntityGetter,
          searcherModeThreshold: _searcherModeThreshold);
      ...
      expect(
          getSelectorBtnLabel(tester),
          Localizator.defaultLocalization
              .constsUnselectAll(overallItems.toString()));
    });
  }
  ...
  Future<List<String>> testSwitchingToSearchMode(WidgetTester tester,
      ...
      bool shouldKeepInSearchMode = false}) async {
    final storage = await _pumpScreenWithEnoughItems(tester,
        newEntityGetter: newEntityGetter,
        searcherModeThreshold: _searcherModeThreshold,
    ...
    indexes.forEach((i) => assureFilterIndexActiveness(tester, i));

    if (!shouldKeepInSearchMode) {
      await deactivateSearcherMode(assistant);
  ...
  Future<BaseStorage<TEntity>> _pumpScreenWithEnoughItems(WidgetTester tester,
          {required TEntity Function(int) newEntityGetter,
          required int searcherModeThreshold,
          Future<int> Function()? itemsLengthGetter,
          int? itemsNumber}) =>
  ...
  void assureFilterIndexActiveness(WidgetTester tester, String index,
      {bool isActive = false}) {
    final activeIndexBoxFinder = find.ancestor(
        of: _findFilterIndex(index, shouldFind: true),
        matching: find.byType(Container));
    expect(
        tester.widgetList<Container>(activeIndexBoxFinder).singleWhereOrNull(
                (Container w) =>
  ...
  Future<void> chooseFilterIndex(
      WidgetAssistant assistant, String index) async {
  ...
  String getSelectorBtnLabel(WidgetTester tester) => tester
  ...
  Future<void> deleteSelectedItems(WidgetAssistant assistant,
    ...
    await _confirmRemovalIfNecessary(assistant,
        ...
        shouldNotWarn: shouldNotWarn);
    await assistant.pumpAndAnimate(3000);

    await assistant.pumpAndAnimate();
  }

  Future<void> selectItemsInEditor(
      WidgetAssistant assistant, List<String> itemTitles) async {
  ...
  Future<void> _deleteItemsUntilSearchIsUnavailable(
    ...
    for (final indexToDelete in filterIndexesToDelete)
      entities.addAll((await tester
          .runAsync(() => storage.fetch(textFilter: indexToDelete)))!);

    await activateEditorMode(assistant);
    await selectItemsInEditor(
        assistant, entities.map((i) => i.textData).toList());
    await deleteSelectedItems(assistant);
  }

  Future<void> _assureTilesDistinctFromFilterIndex(
      WidgetTester tester, String filterIndex) async {
    final tileFinders =
        AssuredFinder.findSeveral(type: ListTile, shouldFind: true);

    final topTiles = tester.widgetList<ListTile>(tileFinders);
    final titles =
        topTiles.map((t) => _extractTitle(tester, t.title!)).toList();
