...
class ExporterTester {
  ...
  Future<void> assertExportedPacks(
      PackStorageMock packStorage, List<StoredPack> packsToExport) async {
    ...
    for (final obj in packObjs) {
      final exportedPackWithCards =
          StoredPack.fromJsonMap(obj as Map<String, dynamic>);
      final exportedPack = exportedPackWithCards.key;
      expect(exportedPack.id, null);

      final originalPack =
          packsToExport.singleWhere((p) => p.name == exportedPack.name);
      assertPacksAreEqual(exportedPack, originalPack);

      final exportedCards = exportedPackWithCards.value;
      final originalCards =
          packedCards.where((c) => c.packId == originalPack.id).toList();
      ...
      exportedCards.forEach((cardDescr) {
        ...
        final card =
            originalCards.singleWhere((c) => c.text == exportedCard.text);
  ...
  static List<StoredPack> getPacksForExport(PackStorageMock packStorage) {
    ...
    return [
      firstPack,
      secondPack,
      new StoredPack('A_${newPack.name}',
          cardsNumber: 0, from: newPack.from, to: newPack.to, id: newPack.id)
    ];
  }

  Future<void> assertImport(
      PackStorageMock packStorage, List<StoredPack> originalPacks) async {
    ...
    for (final originalPack in originalPacks) {
      ...
      originalCards.forEach((originalCard) {
        final importedCard =
            importedCards.singleWhere((c) => c.text == originalCard.text);
    ...
    final storedCards = await packStorage.wordStorage
        .fetchFiltered(parentIds: imports.keys.map((p) => p.id).toList());
    final importedCards = imports.values.expand((cards) => cards);
    expect(importedCards.length, storedCards.length);

    storedCards.forEach((storedCard) => ExporterTester.assertCardsAreEqual(
        importedCards.singleWhere((c) => c.id == storedCard.id), storedCard));
  }

  static String writeToJsonFile(dynamic obj) {
    final dir = ContextChannelMock.getExternalStoragePath();
